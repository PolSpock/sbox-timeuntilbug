...
public class PlayerController : Component
{
	...
	protected override void OnUpdate()
	{
		...
		if ( TimeUntilUpdate )
		{
			Log.Info( "1s passed: OnUpdate AmountTick " + AmountTick );
			AmountTick = 0;

			TimeUntilUpdate = 1.0f;
		}

		if ( !cc.Velocity.AlmostEqual( Vector3.Zero, 0.1f ) )
		{
			Log.Info( cc.Velocity.Length );
		}
	...
	int AmountFixedTick = 0;
	TimeUntil TimeUntilFixedUpdate = 1f;
	...
	protected override void OnFixedUpdate()
	{
		...
		if ( Input.Pressed( "attack1" ) )
		{
			...
			Log.Info( "Game.ActiveScene.FixedUpdateFrequency " + Game.ActiveScene.FixedUpdateFrequency );
			Log.Info( "Game.ActiveScene.NetworkFrequency " + Game.ActiveScene.NetworkFrequency );
			Log.Info( "Game.ActiveScene.NetworkInterpolation " + Game.ActiveScene.NetworkInterpolation );
		}

		AmountFixedTick++;

		if ( TimeUntilFixedUpdate )
		{
			Log.Info( "1s passed: OnFixedUpdate AmountFixedTick " + AmountFixedTick );
			AmountFixedTick = 0;

			TimeUntilFixedUpdate = 1.0f;
		}
